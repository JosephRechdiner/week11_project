...
def create_new_contact(new_document: ContactCreate, collection_name = Depends(CONNECTION.get_collection_name), db= Depends(CONNECTION.get_db)):
...
def update_existing_contact(id, new_document: ContactUpdate, collection_name = Depends(CONNECTION.get_collection_name), db= Depends(CONNECTION.get_db)):
...
def delete_contact(id, collection_name = Depends(CONNECTION.get_collection_name), db= Depends(CONNECTION.get_db)):
